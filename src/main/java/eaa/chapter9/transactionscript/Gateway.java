...
public class Gateway {
	...
	private static final String FIND_PURCHASES_BY_PERSON = 
		" SELECT id,created_at,gallons,reward_gallons " +
		"  FROM purchases " +
		"  WHERE person_id = ? order by created_at asc";
	
	private static final String FIND_REWARDS_BY_PERSON =
		" SELECT id,created_at,gallons " +
		"  FROM rewards " +
		"  WHERE person_id = ? order by created_at asc";
	
	private static final String FIND_REWARD_PLAN_BY_PERSON =
		" SELECT id,plan_id " +
		"  FROM people " +
		"  WHERE id=? ";
	
	private static final String CREATE_PURCHASE =
		" INSERT INTO purchases (id,person_id,created_at,gallons,reward_gallons) " +
		" VALUES (?,?,current_timestamp,?,?) ";
	
	private static final String GET_MAX_PURCHASE_ID =
		" SELECT MAX(id) FROM purchases ";
	
	private static final String CREATE_REWARD = 
		" INSERT INTO rewards (id,person_id,created_at,gallons)" +
		" VALUES (?,?,current_timestamp,?) ";
	
	private static final String GET_MAX_REWARD_ID = 
		" SELECT MAX(id) FROM rewards ";
	
	private static final String GET_REWARD_PLAN_BY_ID = 
		" SELECT purchase_requirement,reward_gallons " +
		"  FROM plans" +
		"  WHERE id=? ";
