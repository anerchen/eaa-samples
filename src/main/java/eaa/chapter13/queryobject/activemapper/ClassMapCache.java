...
import eaa.chapter13.queryobject.annotations.HasMany;
...
class ClassMapCache {
	...
	public ClassMapCache( Class<? extends ActiveRecord> baseClass ) {
		...
		ClassMapCache.checkNull( tableNameAttr, "Class " + baseClass.getCanonicalName() + " does not have the " + MappedTable.class.getCanonicalName() + " attribute present.");
		tableName = tableNameAttr.value();
		ClassMapCache.checkNull( tableName, "Class " + baseClass.getCanonicalName() + " is a mapped table, but has not associated table." );
