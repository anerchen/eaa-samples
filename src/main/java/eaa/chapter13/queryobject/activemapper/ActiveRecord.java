...
public abstract class ActiveRecord {
	...
	private static Map<Class<? extends ActiveRecord>, ClassMapCache> classMappings =
		Collections.synchronizedMap( 
			new HashMap<Class<? extends ActiveRecord>, ClassMapCache>() );
	
	protected ActiveRecord() {
		this.id = NEW_OBJECT;
	}
	
	protected ActiveRecord( long id ) {
		this.id = id;
	}
	...
	private static <T extends ActiveRecord> ClassMapCache mapClass( Class<T> ar_class ) {
	...
	private static <T extends ActiveRecord> String buildWhereClause( Query<T> query, ClassMapCache classMap ) {
		StringBuffer b = new StringBuffer();
		
		int count = 0;
		for( Criteria criteria : query.criteria() ) {
			String objectField = criteria.getField();
			String column = ClassMapCache.checkNull( classMap.getMappedFields().get( objectField ), 
					                                 "Field '" + objectField + "' cannot be querried on, it is not a mapped field in class " + classMap.getBaseClass().getCanonicalName()  );
			if ( count > 0 ) { b.append(" and "); }
			b.append( " " + column + " " + criteria.getSqlOperator() + " ? " );
			count++;
		}
	...
	protected static <T extends ActiveRecord> List<T> find( Query<T> queryObject, Class<T> ar_class ) {
		ClassMapCache classMapping = mapClass( ar_class );
		
		List<T> list = new ArrayList<T>();
		try {
			String selectSQL = "SELECT " + ID_COLUMN + ", " + joinValues( classMapping.getMappedFields().values(), ", " ) + " FROM "
			                   + classMapping.getTableName() + " WHERE " + buildWhereClause( queryObject, classMapping );
			
			PreparedStatement stmt = DB.prepare( selectSQL );
			int param = 1;
			for( Criteria criteria : queryObject.criteria() ) {
				stmt.setObject( param++, criteria.getValue() );
			}			
			ResultSet results = stmt.executeQuery();
			
			while( results.next() ) {
				T object = ar_class.newInstance();
				object.id = results.getLong( ID_COLUMN );
				
				load( object, ar_class, results, classMapping );
			...
			throw new ApplicationException("Error performing query on class " + ar_class.getCanonicalName(), e );
		}
		
		return list;
	}

	protected static <T extends ActiveRecord> T find( Class<T> ar_class, long id ) {
		ClassMapCache classMapping = mapClass( ar_class );
	...
	private static <T extends ActiveRecord> void load( T object, Class<T> ar_class, ResultSet results, ClassMapCache classMapping ) throws SQLException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		// go over the fields
		...
		for( String field : classMapping.getHasManys().keySet() ) {
			...
			Class<? extends ActiveRecord> subClass = resolveClass( ar_class, subClassName );
			...
			List<? extends ActiveRecord> subList = find( subClass, id_column + "=?", object.id );
			if ( belongsToSetter != null ) {
				for( ActiveRecord listObject : subList ) {
	...
	private static Class<? extends ActiveRecord> classNameIsActiveRecord( String name ) {
		try {
			Class clazz = ActiveRecord.class.getClassLoader().loadClass(name);
			Object o = clazz.newInstance();
			if ( o instanceof ActiveRecord ) {
				// this class is good 
				return (Class<? extends ActiveRecord>) clazz;
			}
			// check inheritence
		} catch ( Exception ex ) {
		}
	...
	private static Class<? extends ActiveRecord> resolveClass( Class<? extends ActiveRecord> parent, String subName ) {
		...
		Class<? extends ActiveRecord> clazz = classNameIsActiveRecord( classNameToTry );
	...
	protected static <T extends ActiveRecord> List<T> find( Class<T> ar_class,  String conditions, Object ...params ) {
	...
	public long insert() {
		...
		try {
			...
			for( String key : classMapping.getBelongsTos().keySet() ) {
				...
				Class<? extends ActiveRecord> parentClass = resolveClass( this.getClass(), parentClassName );
			...
			for( String key : classMapping.getBelongsTos().keySet() ) {
				...
				ActiveRecord parentRecord = (ActiveRecord) getterMethod.invoke( this );
