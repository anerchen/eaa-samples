...
class ClassMapCache {
  ...
  private Class<? extends ActiveRecord> baseClass;
  ...
  public ClassMapCache(Class<? extends ActiveRecord> baseClass) {
    this.baseClass = baseClass;

    // get the table name
    MappedTable tableNameAttr = baseClass.getAnnotation(MappedTable.class);
    ClassMapCache.checkNull(tableNameAttr, "Class " + baseClass.getCanonicalName()
        + " does not have the " + MappedTable.class.getCanonicalName() + " attribute present.");
    tableName = tableNameAttr.value();
    ClassMapCache.checkNull(tableName, "Class " + baseClass.getCanonicalName()
        + " is a mapped table, but has not associated table.");
    ...
    for (Field field : declaredFields) {
      ...
      if (mappedField != null) {
        ...
        checkNull(columnName, "On class " + this.baseClass.getCanonicalName() + ", field "
            + fieldName + " is a MappedField, but no DB column is specified.");
      ...
      if (hasMany != null) {
        ...
        checkNull(columnName, "On class " + this.baseClass.getCanonicalName() + ", collection "
            + fieldName + " is in a HasMany relationship, but no subbordinate table is specified.");
      ...
      if (belongsTo != null) {
        ...
        checkNull(className, "On class " + this.baseClass.getCanonicalName() + ", field "
            + fieldName + " is a BelongsTo relationship, but not owning class was specified.");
  ...
  public Method getSetterMethod(String field) {
    return checkNull(setterMethods.get(field), "On class " + this.baseClass.getCanonicalName()
        + ", requested field '" + field + "' has no, or an incorrectley named, setter method.");
  }

  public Method getGetterMethod(String field) {
    return checkNull(getterMethods.get(field), "On class " + this.baseClass.getCanonicalName()
        + ", requested field '" + field + "' has no, or an incorrectley named, getter method.");
  }
  ...
  public String getIdentityColumnOfHasManyFromListClass(Class<? extends ActiveRecord> child) {
    for (HasMany hasMany : hasManys.values()) {
      if (hasMany.listClass().equals(child.getSimpleName())
          || hasMany.listClass().equals(child.getCanonicalName())) {
  ...
  public Class<? extends ActiveRecord> getBaseClass() {
    return baseClass;
  }
